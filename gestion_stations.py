...
class GestionStations:
    ...
    def modifier_station(self):
        ...
        try:
            ...
            new_id_ligne = simpledialog.askinteger(
                "Modifier Ligne",
                f"ID de la ligne:\n{ligne_str}",
            ...
            cursor.execute(
                "UPDATE station SET libelle_station=%s, latitude=%s, longitude=%s WHERE id_station=%s",
                (new_libelle, new_lat, new_long, id_station)
            )

            cursor.execute(
                "UPDATE ligne_station SET id_ligne=%s WHERE id_station=%s",
                (new_id_ligne, id_station)
            )
