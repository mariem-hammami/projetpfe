...
class LoginWindow(tk.Tk):
    def __init__(self):
        ...
        self.configure(bg="black")  # fond noir SRTB

        # ----- Logo SRTB -----
        try:
            logo_image = Image.open("srtb_logo.png.png")
            logo_image = logo_image.resize((150, 150))
            self.logo = ImageTk.PhotoImage(logo_image)
            tk.Label(self, image=self.logo, bg="black").pack(pady=10)
        except:
            tk.Label(self, text="SRTB", font=("Arial", 24, "bold"), fg="#FF8C00", bg="black").pack(pady=10)

        # ----- Champs login -----
        tk.Label(self, text="Nom d'utilisateur", fg="white", bg="black").pack(pady=5)
        self.username_entry = tk.Entry(self, font=("Arial", 12))
        self.username_entry.pack(pady=5, ipady=5)

        tk.Label(self, text="Mot de passe", fg="white", bg="black").pack(pady=5)
        self.password_entry = tk.Entry(self, font=("Arial", 12), show="*")
        self.password_entry.pack(pady=5, ipady=5)

        # ----- Bouton login -----
        tk.Button(self, text="Se connecter", bg="#FF8C00", fg="black",
                  font=("Arial", 14, "bold"), width=20, command=self.login).pack(pady=20)
