...
class GestionBus:
    def __init__(self):
        ...
        self.tree = ttk.Treeview(self.root, columns=("matricule", "type", "modele"), show="headings")
    ...
    def load_bus(self):
        ...
        try:
            conn = connect_db()
            cursor = conn.cursor()
            cursor.execute("SELECT matricule_vehicule, type_vehicule, modele_vehicule FROM vehicule")
    ...
    def ajouter_bus(self):
        ...
        if matricule and type_bus and modele:
            try:
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("INSERT INTO vehicule (matricule_vehicule, type_vehicule, modele_vehicule) VALUES (%s,%s,%s)",
                               (matricule, type_bus, modele))
                conn.commit()
                conn.close()
    ...
    def modifier_bus(self):
        ...
        if new_type and new_modele:
            try:
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("UPDATE vehicule SET type_vehicule=%s, modele_vehicule=%s WHERE matricule_vehicule=%s",
                               (new_type, new_modele, matricule))
                conn.commit()
                conn.close()
