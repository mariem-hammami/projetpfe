...
class GestionLignes:
    ...
    def modifier_ligne(self):
        ...
        new_libelle = simpledialog.askstring("Modifier Ligne", "Nouveau libellé de la ligne:", initialvalue=item["values"][1])
        new_id_agence = simpledialog.askinteger("Modifier Ligne", "Nouvel ID Agence:", initialvalue=item["values"][2])
        if new_libelle and new_id_agence is not None:
            try:
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("UPDATE ligne SET libelle_ligne=%s, id_agence=%s WHERE id_ligne=%s",
    ...
    def supprimer_ligne(self):
        ...
        confirm = messagebox.askyesno("Supprimer Ligne", f"Voulez-vous vraiment supprimer la ligne {item['values'][1]} ?")
        if confirm:
            try:
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM ligne WHERE id_ligne=%s", (id_ligne,))
                conn.commit()
                conn.close()
